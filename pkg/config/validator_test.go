...
func TestFileExists(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			path: "exists.json",
			setup: func() {
				os.WriteFile("exists.json", []byte("content"), 0644)
			},
			expected: true,
		},
		{
...
func TestValidateConfigStructure(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			cfg:      &Config{},
			expected: []string{"baseImage is required", "at least one image must be specified"},
		},
		{
			name:     "invalid platform",
			cfg:      &Config{BaseImage: "valid/image", Images: []string{"valid/image"}, Platforms: []string{"invalid/platform"}},
			expected: []string{"invalid platform format: invalid/platform"},
		},
		{
			name:     "valid configuration",
			cfg:      &Config{BaseImage: "valid/image", Images: []string{"valid/image"}, Platforms: []string{"linux/amd64"}},
			expected: []string{},
		},
	}
...
func TestLoadDefaultConfig(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "default config file does not exist",
			setup:   func() {},
			wantErr: true,
